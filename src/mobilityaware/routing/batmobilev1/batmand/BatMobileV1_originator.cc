...
BatMobileV1_NeighNode * BatMobileV1::create_neighbor(BatMobileV1_OrigNode *orig_node, BatMobileV1_OrigNode *orig_neigh_node, const ManetAddress &neigh, BatMobileV1_BatmanIf *if_incoming)
{
    return new BatMobileV1_NeighNode(orig_node, orig_neigh_node, neigh, if_incoming, num_words, global_win_size);
}

/* this function finds or creates an originator entry for the given address if it does not exits */
BatMobileV1_OrigNode *BatMobileV1::get_orig_node(const ManetAddress &addr) {
    BatMobileV1_OrigNode *orig_node;
    BatMobileV1_OrigMap::iterator it;
    ...
    orig_node = new BatMobileV1_OrigNode();
    orig_node->bcast_own.resize(found_ifs * num_words);
    orig_node->bcast_own_sum.resize(found_ifs);
    orig_node->clear();
    orig_node->orig = addr;
    origMap.insert(std::pair<ManetAddress, BatMobileV1_OrigNode *>(addr, orig_node));
    return orig_node;
}

void BatMobileV1::update_orig(BatMobileV1_OrigNode *orig_node, BatMobileV1_BatmanPacket *in, const ManetAddress &neigh, BatMobileV1_BatmanIf *if_incoming, BatMobileV1_HnaElement *hna_recv_buff, int16_t hna_buff_len, uint8_t is_duplicate, const simtime_t &curr_time) {
    BatMobileV1_GwNode *gw_node;
    BatMobileV1_NeighNode *neigh_node = NULL, *tmp_neigh_node = NULL, *best_neigh_node = NULL;
    ...
    if (neigh_node == NULL) {
        neigh_node = new BatMobileV1_NeighNode(orig_node, get_orig_node(neigh), neigh, if_incoming, num_words, global_win_size);
    } else {
...
/*     is_new_seqno = bit_get_packet(neigh_node->seq_bits, in->seqno - orig_node->last_seqno, 1);
    ...
    NodeEntry *destination = m_nodeMap.getDestinationNodeEntry(in->getOrig());
    if(destination)
    {
        NeighborNodeEntry *bestNeighbor = destination->getBestNeighbor();
        if(bestNeighbor)
        {
            if(bestNeighbor->getAddress().str()==neigh.str())
                best_neigh_node = neigh_node;
            else
            {
                best_neigh_node = 0;

                // node aus id bestimmen

                for (unsigned int list_pos = 0; list_pos < orig_node->neigh_list.size(); list_pos++)
                {  // gehe alle bekannten Nachbarknoten auf dem Weg zu orig_node durch
                    tmp_neigh_node = orig_node->neigh_list[list_pos];
                    if(tmp_neigh_node->addr.str()==bestNeighbor->getAddress().str())
                    {
                        best_neigh_node = tmp_neigh_node;
                        break;
                    }
    ...
    if(best_neigh_node)
    {
        //changeRoute(orig_node->orig, best_neigh_node->addr, 1);
        update_routes(orig_node, best_neigh_node, hna_recv_buff, hna_buff_len);
    }


    // TODO: compare changeRoute to update_routes
...
void BatMobileV1::purge_orig(const simtime_t &curr_time)
{
    BatMobileV1_OrigMap::iterator it;
    BatMobileV1_OrigNode *orig_node;
    BatMobileV1_NeighNode *neigh_node, *best_neigh_node;
    BatMobileV1_GwNode *gw_node;
    ...
    for (it = origMap.begin(); it != origMap.end();) {
        ...
        if (curr_time > (orig_node->last_valid + (2 * purge_timeout))) {
            ...
            if (it != origMap.begin()) {
                BatMobileV1_OrigMap::iterator itAux = it;
                it++;
                origMap.erase(itAux);
            } else {
                origMap.erase(it);
            ...
            while (!orig_node->neigh_list.empty()) {
                BatMobileV1_NeighNode *node = orig_node->neigh_list.back();
            ...
            {
                BatMobileV1_HnaElement * buffer = new BatMobileV1_HnaElement[orig_node->hna_buff.size()];
...
BatMobileV1_OrigNode::BatMobileV1_OrigNode()
{
    clear();
}

void BatMobileV1_OrigNode::clear()
{
    router = NULL;
    batmanIf = NULL;
    totalRec = 0;
...
std::string BatMobileV1_OrigNode::info() const
{
    ...
    BatMobileV1_NeighNode *neigh_node = NULL;
    for (unsigned int i = 0; i < neigh_list.size(); i++)
    {
        BatMobileV1_NeighNode * tmp_neigh_node = neigh_list[i];
...
std::string BatMobileV1_NeighNode::info() const
{
    std::stringstream out;
    out << "addr:"  << addr << "  ";
...
BatMobileV1_OrigNode::~BatMobileV1_OrigNode()
{
    ...
    {
        BatMobileV1_NeighNode *node = neigh_list.back();
        neigh_list.pop_back();
        node->owner_node = NULL;
        delete node;
    }
...
void BatMobileV1_NeighNode::clear()
{
...
BatMobileV1_NeighNode::~BatMobileV1_NeighNode()
{
    tq_recv.clear();
    real_bits.clear();
    if (owner_node)
...
BatMobileV1_NeighNode::BatMobileV1_NeighNode(BatMobileV1_OrigNode* origNode, BatMobileV1_OrigNode *orig_neigh_node, const ManetAddress &neigh, BatMobileV1_BatmanIf* ifIncoming, const uint32_t &num_words, const uint32_t &global_win_size)
{
