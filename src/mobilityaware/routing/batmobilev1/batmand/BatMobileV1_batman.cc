...
BatMobileV1_BatmanIf *BatMobileV1::is_batman_if(InterfaceEntry *dev)
{
...
void BatMobileV1::choose_gw(void)
{
    BatMobileV1_GwNode *gw_node, *tmp_curr_gw = NULL;
...
void BatMobileV1::update_routes(BatMobileV1_OrigNode *orig_node, BatMobileV1_NeighNode *neigh_node, BatMobileV1_HnaElement *hna_recv_buff, int16_t hna_buff_len)
{
    BatMobileV1_NeighNode *old_router;
...
void BatMobileV1::update_gw_list(BatMobileV1_OrigNode *orig_node, uint8_t new_gwflags, uint16_t gw_port)
{
    ...
    gw_node = new BatMobileV1_GwNode();
...
void BatMobileV1::get_gw_speeds(unsigned char gw_class, int *down, int *up)
{
...
unsigned char BatMobileV1::get_gw_class(int down, int up)
{
...
int BatMobileV1::isBidirectionalNeigh(BatMobileV1_OrigNode *orig_node, BatMobileV1_OrigNode *orig_neigh_node, BatMobileV1_BatmanPacket *in, const simtime_t &recv_time, BatMobileV1_BatmanIf *if_incoming)
{
    BatMobileV1_NeighNode *neigh_node = NULL, *tmp_neigh_node = NULL;
    uint8_t total_count;
...
uint8_t BatMobileV1::count_real_packets(BatMobileV1_BatmanPacket *in, const ManetAddress &neigh, BatMobileV1_BatmanIf *if_incoming)
{
    BatMobileV1_OrigNode *orig_node;
    BatMobileV1_NeighNode *tmp_neigh_node;
...
void BatMobileV1::handleMobilityData(BatMobileV1_BatmanPacket *_packet, const ManetAddress &_forwarder)
{
    if(p_locationService)
    {
        //debugPacket(_packet, _forwarder);
...
void BatMobileV1::updateRoutingTable()
{
    ...
    for(unsigned int i=0; i<entries.size(); i++)
    {
        ...
        if(neighbor) // update the os routing table
        {
            //changeRoute(destination->getAddress(), neighbor->getAddress(), 1);

            if(m_address.str()=="145.236.0.1" && 1==2)
            {
                std::cout << "***************" << std::endl;
                std::cout << destination->getAddress().str() << " via " << neighbor->getAddress() << " score:" << neighbor->getScore() << std::endl;
                std::cout << "***************" << std::endl;
            }
        }
        else // no route to destination - delete routing table entry
        {
            //invalidateRoute(destination->getAddress());
        }
    }
}
...
*/
void BatMobileV1::parseIncomingPacket(ManetAddress neigh, BatMobileV1_BatmanIf *if_incoming, BatMobileV1_BatmanPacket *bat_packet)
{
    BatMobileV1_OrigNode *orig_neigh_node, *orig_node;
    simtime_t curr_time;
    BatMobileV1_HnaElement *hna_recv_buff;
...

    {
        ...
        for ( ; bat_packet; bat_packet = next_packet ? check_and_cast<BatMobileV1_BatmanPacket*>(next_packet) : NULL) {
            next_packet = bat_packet->decapsulate();

            EV << "packet receive from :" << bat_packet->getOrig() << endl;

            handleMobilityData(bat_packet, neigh);
            ...
            delete bat_packet;
            bat_packet = next_packet ? check_and_cast<BatMobileV1_BatmanPacket*>(next_packet) : NULL;
        }
        delete next_packet;
    }
}
