...
void BatMobileV1::schedule_own_packet(BatMobileV1_BatmanIf *batman_if)
{
    BatMobileV1_ForwNode *forw_node_new = NULL;
    BatMobileV1_OrigNode *orig_node;

    debug_output(4) << "schedule_own_packet(): " << batman_if->dev << "\n";

    forw_node_new = new BatMobileV1_ForwNode();
    ...
    //((BatMobileV1_BatmanPacket *)forw_node_new->pack_buff)->seqno = htons(((BatMobileV1_BatmanPacket *)forw_node_new->pack_buff)->seqno);


    BatMobileV1_Forwlist::iterator forwListIt;
    for (forwListIt = forw_list.begin(); forwListIt != forw_list.end(); forwListIt++) {
        BatMobileV1_ForwNode *forw_packet_tmp = *forwListIt;
...
//        forw_packet_tmp = NULL;
    ...
    for (BatMobileV1_OrigMap::iterator it = origMap.begin(); it != origMap.end(); ++it)
    {
        orig_node = it->second;
...
void BatMobileV1::schedule_forward_packet(BatMobileV1_OrigNode *orig_node, BatMobileV1_BatmanPacket *in, const ManetAddress &neigh, uint8_t directlink, int16_t hna_buff_len, BatMobileV1_BatmanIf *if_incoming, const simtime_t &curr_time)
{
    BatMobileV1_ForwNode *forw_node_new = NULL, *forw_node_aggregate = NULL, *forw_node_pos = NULL;
    //struct list_head *list_pos = forw_list.next, *prev_list_head = (struct list_head *)&forw_list;
    BatMobileV1_BatmanPacket *bat_packet;
    uint8_t tq_avg = 0;
    simtime_t send_time;
    ...
    BatMobileV1_Forwlist::iterator  it;
...
void BatMobileV1::send_outstanding_packets(const simtime_t &curr_time)
{
    BatMobileV1_ForwNode *forw_node;
    ...
    while (true) {
        ...
        for (unsigned int i = 0; i<if_list.size(); i++) {
            ...
            BatMobileV1_BatmanPacket *bat_packetAux = bat_packet;
...
//                    addr_to_string(bat_packet->orig, orig_str, ADDR_STR_LEN);
                ...
                bat_packetAux = const_cast<BatMobileV1_BatmanPacket *> (dynamic_cast<BatMobileV1_BatmanPacket *>(bat_packetAux->getEncapsulatedPacket()));
            }
